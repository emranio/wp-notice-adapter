...
class WP_Notice_Adapter{
    ...
    public function get_notice(){
        ...
        global $emranioim_lib_notice_list;

        if(!isset($emranioim_lib_notice_list[$this->notice_id])){
            $emranioim_lib_notice_list[$this->notice_id] = __FILE__;
    ...
    public function  generate_html() {
        ...
        <div 
            ...
            class="notice emranio-notice notice-<?php echo esc_attr($this->notice_id . ' ' .$this->class); ?> <?php echo (false === $this->dismissible ? '' : 'is-dismissible') ;?>"
                    ...
                    <?php if(!empty($this->buttons)): ?>
                        <div class="submit">
                            <?php foreach($this->buttons as $button): ?>
                                <a id="<?php echo (!isset($button['id']) ? '' : $button['id']); ?>" href="<?php echo esc_url($button['url']); ?>" class="emranio-notice-buttons <?php echo esc_attr($button['class']); ?>">
    ...
    public static function init(){
        add_action( 'wp_ajax_emranio-notices', [ __CLASS__, 'dismiss_ajax_call' ] );
		...
		echo "
			<script>
                jQuery(document).ready(function ($) {
                    $( '.emranio-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {

                        _this 		        = $( this ).parents('.emranio-notice').eq(0);
            ...
            <style>
                .emranio-notice .notice-icon{
                    display:inline-block;
                }

                .emranio-notice .notice-icon:before{
                    vertical-align: middle!important;
                    margin-top: -1px;
                }

                .emranio-notice .main-message{
                    margin-bottom: 10px;
                }

                .emranio-notice-buttons {
                    text-decoration:none;
                }

                .emranio-notice-buttons > i{
                    margin-right: 3px;
                }

                .emranio-notice .notice-right-container{
                    /* padding-top: 10px; */
                }
                .emranio-notice .notice-right-container .submit {
                    padding-top: 0;
                    margin-top: 0;
                    margin-bottom: 15px;
                    padding-bottom: 0;
                ...
                .emranio-notice.no-gutter{
                    padding: 0!important;
                    border: 0!important;
                }

                .emranio-notice.no-gutter .notice-right-container{
                    padding: 0!important;
                    margin: 0!important;
                }
